...
test('profitability copy uses promised delivery wording, not actual delivery', () => {
  ...
  assert.match(HOME_PROFITABILITY_SUBTITLE, /período/)
  assert.doesNotMatch(HOME_PROFITABILITY_SUBTITLE, /entregad[oa]s? en el mes/i)
  assert.match(HOME_PAGE_SUBTITLE, /entrega programada/)
  assert.match(HOME_PERIOD_SUBTITLE, /entrega programada/)
